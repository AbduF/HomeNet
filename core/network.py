...
class NetworkScanner:
    """Network scanner for discovering hosts - Optimized for Raspberry Pi 3."""

    def __init__(self):
        self.logger = logging.getLogger("HomeNet.NetworkScanner")
        self.cache = {}  # Cache scan results
    ...
    def full_scan(self, use_cache=True):
        """Perform full network scan - Optimized for Pi 3."""
        import time

        # Check cache first (reduce CPU usage)
        current_time = time.time()
        if use_cache and (current_time - self.last_scan_time) < self.cache_timeout:
            self.logger.debug("Using cached scan results")
            return list(self.cache.values())

        self.logger.info("Starting network scan...")

        # Fast ARP scan only (skip slow nmap)
        ...
        # Update cache
        self.cache = {h.get('mac_address'): h for h in hosts}
