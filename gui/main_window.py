...
class MainWindow(ctk.CTkFrame):
    ...
    def show_view(self, view_name):
        ...
        if view_class:
            try:
                # Special handling for AI Help
                if view_name == 'ai_help':
                    from .ai_help import AIHelpView
                    from core.ai_assistant import AIAssistant
                    ai = AIAssistant(self.db, self.config)
                    self.current_view = AIHelpView(
                        self.content_frame,
                        self.db,
                        self.config,
                        ai,
